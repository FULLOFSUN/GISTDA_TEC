...
# ~213-station Thai CORS network. Run independently of the TPS/IDW
# scripts to keep RAM usage down (one method per process).
#
# NOTE: Kriging is meaningfully slower than TPS/IDW (variogram fitting
# cost grows with station count). Test with a small DOY range and a
# larger HOUR_STEP first before committing to the full DOY 14-30 run.
#
# Requires: pip install pykrige --break-system-packages
#
# Usage:
...
PLOT_OUT_DIR   = TEC_OUTPUT_DIR / "TEC_maps_comparison"
...
def collect_hour_points(records: list, h_utc: int, window_h: float = 1.0):
    ...
    for rec in records:
        ...
        if np.isfinite(med) and med > 0:
            lats.append(rec["lat"]); lons.append(rec["lon"]); vals.append(float(med))
...
def interpolate_ok(lats, lons, vals, lat_g, lon_g):
    ok = OrdinaryKriging(lons, lats, vals, variogram_model="spherical",
                         verbose=False, enable_plotting=False)
    z, _ss = ok.execute("grid", lon_g, lat_g)
    return np.clip(np.asarray(z), VTEC_MIN, VTEC_MAX)
...
def plot_day_map(doy, records, thai_paths, grid_info):
    ...
    for panel_i, h_utc in enumerate(hour_bins):
        ...
        if len(vals) >= 4:
            try:
                z = interpolate_ok(lats, lons, vals, lat_g, lon_g)
            except Exception as e:
