...
METHOD = "idw"
METHOD_LABEL = "Inverse Distance Weighting"
...
def interpolate_idw(lats, lons, vals, grid_pts, LAT_G, power=2):
    pts = np.column_stack([lats, lons])
    d = cdist(grid_pts, pts)
    d[d == 0] = 1e-10
    w = 1.0 / (d ** power)
    z = (w @ vals) / w.sum(axis=1)
    return np.clip(z.reshape(LAT_G.shape), VTEC_MIN, VTEC_MAX)
...
def plot_day_map(doy, records, thai_paths, grid_info):
    ...
    for panel_i, h_utc in enumerate(hour_bins):
        ...
        if len(vals) >= 4:
            try:
                z = interpolate_idw(lats, lons, vals, grid_pts, LAT_G)
            except Exception as e:
