...
NAV_DIR  = BASE_DIR / "RINEX3_Nav_File"   # EDIT: where brdc/BRDC nav files live
...
# RUN CONFIG
# =============================================================================
DOY_START = 19    # change to process a single day (DOY_START == DOY_END)
DOY_END   = DOY_START  # change to process a range of days (e.g. DOY_START=14, DOY_END=16)
YEAR      = 2026
YEAR_2D   = "26"
...
def process_one(station: str, doy: int) -> dict | None:
    ...
    if obs_file is None:
        ...
        return None
    if nav_file is None:
        print(f"  [skip] {station} DOY{doy:03d} — nav file not found")
        return None
    ...
    if DRY_RUN:
        print(f"  [dry-run] {station} DOY{doy:03d}  lat={lat:.4f} lon={lon:.4f}  "
              f"obs={obs_file.name}  nav={nav_file.name}")
        return {
            "station": station,
            "lat":     lat,
            "lon":     lon,
    ...
    print(f"  -> {station} DOY{doy:03d}  lat={lat:.4f} lon={lon:.4f}  "
          f"obs={obs_file.name}  nav={nav_file.name}")
    try:
        # 1. Read RINEX files (rinex_reader auto-detects 2.11 vs 3.04)
        obs, nav, doy_r, year, month, day, stn = read_rinex211(
            str(obs_file), str(nav_file)
        )
        if obs is None:
            print(f"  [fail] {station} DOY{doy:03d} — RINEX read failed")
            return None
        ...
        result = calculate_tec(obs, nav, doy_r, satb, str(out_dir))
        if result is None:
            return None
